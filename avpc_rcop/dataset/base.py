...
class BaseDataset(torchdata.Dataset):
    ...
    def _mix_n_and_stft(self, audios, mult_mix=False):
        N = len(audios)
        mags = [None for n in range(N)]

        if mult_mix:   # mix first audio with remaining audios, respectively
            amp_mix = [None for n in range(1, N)]
            phase_mix = [None for n in range(1, N)]
            for n in range(1, N):
                # mix
                audio_mixN = np.asarray([audios[0], audios[n]]).sum(axis=0) / 2
                # STFT
                amp_mix[n - 1], phase_mix[n - 1] = self._stft(audio_mixN)
                amp_mix[n - 1] = amp_mix[n - 1].unsqueeze(0)
                phase_mix[n - 1] = phase_mix[n - 1].unsqueeze(0)
        else:
            # mix
            audio_mix = np.asarray(audios).sum(axis=0) / N
            # STFT
            amp_mix, phase_mix = self._stft(audio_mix)
            amp_mix = amp_mix.unsqueeze(0)
            phase_mix = phase_mix.unsqueeze(0)

        for n in range(N):
            ampN, _ = self._stft(audios[n])
            mags[n] = ampN.unsqueeze(0)

        return amp_mix, mags, phase_mix
