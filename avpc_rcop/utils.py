...
def plot_loss_metrics(path, history, include_pretrain=False):

    if include_pretrain:
        # plot loss of SimSiam
        fig = plt.figure()
        ...
        plt.legend()
        fig.savefig(os.path.join(path, 'pt_loss_simsiam.png'), dpi=200)
        plt.close('all')

    else:
        # plot mask prediction error
        fig = plt.figure()
        plt.plot(history['train']['epoch'], history['train']['loss'],
                 color='b', label='training')
        plt.plot(history['val']['epoch'], history['val']['loss'],
                 color='c', label='validation')
        plt.legend()
        fig.savefig(os.path.join(path, 'ft_loss_mask.png'), dpi=200)
        plt.close('all')
...
def output_visuals(batch_data, outputs, args):
    ...
    for j in range(B):
        ...
        prefix = '+'.join(prefix)
        mkdir(os.path.join(args.vis_ft, prefix))
        ...
        wavfile.write(os.path.join(args.vis_ft, filename_mixwav), args.audRate, mix_wav.astype(np.float32))   # np.float64 is not supported
        Image.fromarray(mix_amp[::-1, :, :]).save(os.path.join(args.vis_ft, filename_mixmag))
        Image.fromarray(weight[::-1, :]).save(os.path.join(args.vis_ft, filename_weight))
        ...
        for n in range(N):
            ...
            Image.fromarray(gt_mask[::-1, :]).save(os.path.join(args.vis_ft, filename_gtmask), "PNG")
            Image.fromarray(pred_mask[::-1, :]).save(os.path.join(args.vis_ft, filename_predmask), "PNG")
            ...
            Image.fromarray(gt_mag[::-1, :, :]).save(os.path.join(args.vis_ft, filename_gtmag), "PNG")
            Image.fromarray(pred_mag[::-1, :, :]).save(os.path.join(args.vis_ft, filename_predmag), "PNG")

            # output audio
            filename_gtwav = os.path.join(prefix, 'gt{}.wav'.format(n + 1))
            filename_predwav = os.path.join(prefix, 'pred{}.wav'.format(n + 1))
            wavfile.write(os.path.join(args.vis_ft, filename_gtwav), args.audRate, gt_wav.astype(np.float32))
            wavfile.write(os.path.join(args.vis_ft, filename_predwav), args.audRate, preds_wav[n].astype(np.float32))

        #     # output video
        #     frames_tensor = [recover_rgb(frames[n][j, :, t]) for t in range(args.num_frames)]
        #     frames_tensor = np.asarray(frames_tensor)
        #     path_video = os.path.join(args.vis_ft, prefix, 'video{}.mp4'.format(n + 1))
        ...
        #     combine_video_audio(
        #         path_video,
        #         os.path.join(args.vis_ft, filename_gtwav),
        #         os.path.join(args.vis_ft, filename_av))
        #
        #     row_elements += [
        #         {'video': filename_av},
...
def calc_metrics(batch_data, outputs, args):
    ...
    for n in range(N):
        if args.log_freq:
            grid_unwarp = torch.from_numpy(
                warpgrid(B, args.stft_frame // 2 + 1, pred_masks_[0].size(3), warp=False)).cuda(non_blocking=True)
            pred_masks_linear[n] = F.grid_sample(pred_masks_[n], grid_unwarp, align_corners=False)
        else:
            pred_masks_linear[n] = pred_masks_[n]
