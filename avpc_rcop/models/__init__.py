...
from .vision_net import ResNetFC, ResNetDilated, VGGFC
from .audio_net import PCNetLR
from .simsiam_net import SimSiam
...
class ModelBuilder():
    ...
    def build_frame(self, arch='resnet18', fc_vis=512, weights=''):
        ...
        elif arch == 'resnet34fc':
            original_resnet = torchvision.models.resnet34(pretrained)
            net = ResNetFC(original_resnet, fc_vis=fc_vis)
        elif arch == 'vgg16fc':
            original_vgg = torchvision.models.vgg16(pretrained)
            net = VGGFC(original_vgg, fc_vis=fc_vis)
        elif arch == 'vgg19fc':
            original_vgg = torchvision.models.vgg19(pretrained)
            net = VGGFC(original_vgg, fc_vis=fc_vis)
        else:
    ...
    # build for audio
    def build_sound(self, arch='pcnetlr', weights='', cyc_in=4, ngf=64, fc_vis=512, n_fm_out=1):
        if arch == 'pcnetlr':
    ...
    # build for RCoP
    def build_simsiam(self, in_dim_proj, weights=''):
        net = SimSiam(in_dim_proj=in_dim_proj)

        if len(weights) > 0:
            print('Loading weights for net_simsiam')
            net.load_state_dict(torch.load(weights))
